...
export const fetchProducts = (queryString="") => async (dispatch) => {
    try {
        ...
        // productsData = await api.get(`/public/products?pageSize=${data.totalElements}&${queryString}&pageNumber=1`);
...
export const addToCart = (data, qty=1, toast) => (dispatch, getState)=>{
    // Find the product
    // console.log("From ADTOCART: ", getState())
    ...
    if (isQuantityExist) {
        ...
        // console.log("Enter before toast");
        toast.success(`${data?.productName} added to the cart`);
        // console.log("Exit after toast");
...
export const increaseCartQuantity = 
    ...
    async (dispatch, getState) => {
        ...
        const getProduct = productsData.data.content.find(
            (item) => item.productId === data.productId
        );
        // console.log("getState(): ", getState(), " And getProduct: ", getProduct);
...
export const addUpdateUserAddress =
   ...
   const token = localStorage.getItem("auth");
    ...
    try {
        if (!addressId) {
            console.log("Entered into try for address and Data: ", sendData);
            const { data } = await api.post("/addresses", sendData);
            console.log("Address created successfully:", data);
        } else {
            console.log("Entered into else for address update");
...
export const createStripePaymentSecret 
    = (toast, sendData) => async (dispatch, getState) => {
        try {
            dispatch({ type: "IS_FETCHING" });
